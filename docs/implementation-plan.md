...
- [ ] The full onboarding flow works end to end against the fake client and matches the polished reference's states and motion. Build/tests/lint are green and the motion primitives (`EntranceItem`, `pressScale()`, `WatchMotion` curves, segmented-control thumb slide, radar pulse) are wired through the screens, but a textual/structural read against the HTML turned up real copy and layout mismatches (Welcome screen's headline/subtext hierarchy is inverted and drops the "stays on this device" copy; Permissions screen has no progress bar and a different headline; Pair confirmation's warning copy changes the guarantee given to the user; Keep it running drops the manufacturer hint and the live-status pulse; Profile's segmented control defaults to the opposite option order) — see task-14-report.md for the full list. Not checking this box until those are triaged and either fixed or accepted, and until someone confirms it on a real device/browser (no emulator was available in this sandbox).
- [ ] Every screen and state has a `@Preview`. 11 previews across the 7 screens; Permissions (partial/all-granted) and Pairing (in-progress/success/failed) fully cover their documented states, but Find your watch only has "found" and "timeout" previews — it's missing the plain "scanning, no results yet" state called out in the phase's own spec. Leaving unchecked until that preview is added.

Outstanding before this phase can move to `Done`: add the missing "scanning" `@Preview` to `FindWatchScreen.kt`; resolve the copy/layout mismatches above (or explicitly accept the divergences as intentional and note why); and perform the real device/browser check that Task 13/14 could not do in this sandbox — install the app, confirm Welcome shows for an unbound watch and the 4-tab shell after pairing, and open the polished HTML side by side with the running app.
